...
namespace Payske\HttpClient;

/**
 * @internal
 * @covers \Payske\HttpClient\CurlClient
 */
final class CurlClientTest extends \PHPUnit\Framework\TestCase
{
    use \Payske\TestHelper;
    use \Payske\TestServer;
    ...
    {
        $this->origMaxNetworkRetries = \Payske\Payske::getMaxNetworkRetries();
        $this->origMaxNetworkRetryDelay = \Payske\Payske::getMaxNetworkRetryDelay();
        $this->origInitialNetworkRetryDelay = \Payske\Payske::getInitialNetworkRetryDelay();
    }
    ...
    {
        $PayskeReflector = new \ReflectionClass('\Payske\Payske');

        $this->maxNetworkRetryDelayProperty = $PayskeReflector->getProperty('maxNetworkRetryDelay');
        $this->maxNetworkRetryDelayProperty->setAccessible(true);

        $this->initialNetworkRetryDelayProperty = $PayskeReflector->getProperty('initialNetworkRetryDelay');
        $this->initialNetworkRetryDelayProperty->setAccessible(true);

        $curlClientReflector = new \ReflectionClass('\Payske\HttpClient\CurlClient');
    ...
    {
        \Payske\Payske::setMaxNetworkRetries($this->origMaxNetworkRetries);
    ...
    {
        $fakeRandomGenerator = $this->createMock('\Payske\Util\RandomGenerator');
    ...
    public function testDefaultOptions()
    {
        ...
        $this->expectException('Payske\Exception\UnexpectedValueException');
    ...
    public function testShouldRetryOnPayskeShouldRetryTrue()
    {
        \Payske\Payske::setMaxNetworkRetries(2);
        ...
        static::assertTrue($this->shouldRetryMethod->invoke($curlClient, 0, 400, ['Payske-should-retry' => 'true'], 0));
    }

    public function testShouldNotRetryOnPayskeShouldRetryFalse()
    {
        \Payske\Payske::setMaxNetworkRetries(2);
        ...
        static::assertTrue($this->shouldRetryMethod->invoke($curlClient, 0, 500, [], 0));
        static::assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 500, ['Payske-should-retry' => 'false'], 0));
    }
    ...
    public function testShouldNotRetryAtMaximumCount()
    {
        ...
        static::assertFalse($this->shouldRetryMethod->invoke($curlClient, 0, 0, [], \Payske\Payske::getMaxNetworkRetries()));
    }
    ...
    public function testSleepTimeShouldGrowExponentially()
    {
        ...
        static::assertSame(
            \Payske\Payske::getInitialNetworkRetryDelay() * 1,
            $this->sleepTimeMethod->invoke($curlClient, 1, [])
        );
        static::assertSame(
            \Payske\Payske::getInitialNetworkRetryDelay() * 2,
            $this->sleepTimeMethod->invoke($curlClient, 2, [])
        );
        static::assertSame(
            \Payske\Payske::getInitialNetworkRetryDelay() * 4,
            $this->sleepTimeMethod->invoke($curlClient, 3, [])
        );
        static::assertSame(
            \Payske\Payske::getInitialNetworkRetryDelay() * 8,
    ...
    public function testSleepTimeShouldAddSomeRandomness()
    {
        ...
        $baseValue = \Payske\Payske::getInitialNetworkRetryDelay() * (0.5 * (1 + $randomValue));

        // the initial value cannot be smaller than the base,
        // so the randomness is ignored
        static::assertSame(\Payske\Payske::getInitialNetworkRetryDelay(), $this->sleepTimeMethod->invoke($curlClient, 1, []));
    ...
    public function testSetRequestStatusCallback()
    {
        try {
            ...
            \Payske\ApiRequestor::setHttpClient(null);
        }
    }
...
header("Payske-should-retry", "true");
?>
{}
EOF;

        \Payske\Payske::setMaxNetworkRetries(3);
        $this->fastRetries();
...
EOF;
        ...
        static::assertSame('Payske\Exception\ApiConnectionException', \get_class($exception));
    ...
    {
        $curl = new CurlClient();
        $coupon = \Payske\Coupon::retrieve('coupon_xyz');

        $absUrl = \Payske\Payske::$apiBase . '/api/v1/coupons/xyz';
        $opts[\CURLOPT_HTTPGET] = 1;
        $opts[\CURLOPT_URL] = $absUrl;
        $opts[\CURLOPT_HTTPHEADER] = ['Authorization: Basic c2tfdGVzdF94eXo6'];
