...
class ApiRequestor
{
    ...
    private static $OPTIONS_KEYS = ['api_key', 'idempotency_key', 'Payske_account', 'Payske_version', 'api_base'];
    ...
    public function __construct($apiKey = null, $apiBase = null)
    {
        $this->_apiKey = $apiKey;
        if (!$apiBase) {
            $apiBase = Payske::$apiBase;
        }
        $this->_apiBase = $apiBase;
    }

    /**
     * Creates a telemetry json blob for use in 'X-Payske-Client-Telemetry' headers.
     *
     * @static
     *
    ...
    private static function _telemetryJson($requestTelemetry)
    {
        ...
        Payske::getLogger()->error('Serializing telemetry payload failed!');
    ...
    {
        $uaString = 'Payske/v1 PhpBindings/' . Payske::VERSION;

        $langVersion = \PHP_VERSION;
        $uname_disabled = static::_isDisabled(\ini_get('disable_functions'), 'php_uname');
        $uname = $uname_disabled ? '(disabled)' : \php_uname();

        $appInfo = Payske::getAppInfo();
        $ua = [
            'bindings_version' => Payske::VERSION,
            'lang' => 'php',
        ...
        return [
            'X-Payske-Client-User-Agent' => \json_encode($ua),
            'User-Agent' => $uaString,
    ...
    private function _prepareRequest($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $myApiKey = Payske::$apiKey;
        }

        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
                . '"Payske::setApiKey(<API-KEY>)".  You can generate API keys from '
                . 'the Payske web interface.  See https://payske.com/api for '
                . 'details, or email support@payske.com if you have any questions.';

            throw new Exception\AuthenticationException($msg);
        }

        // Clients can supply arbitrary additional keys to be included in the
        // X-Payske-Client-User-Agent header via the optional getUserAgentInfo()
        // method
        $clientUAInfo = null;
        ...
        if (Payske::$apiVersion) {
            $defaultHeaders['Payske-Version'] = Payske::$apiVersion;
        }

        if (Payske::$accountId) {
            $defaultHeaders['Payske-Account'] = Payske::$accountId;
        }

        if (Payske::$enableTelemetry && null !== self::$requestTelemetry) {
            $defaultHeaders['X-Payske-Client-Telemetry'] = self::_telemetryJson(self::$requestTelemetry);
        }
