...
class CurlClient implements ClientInterface, StreamingClientInterface
{
    ...
    /** @var \Payske\Util\RandomGenerator */
     ...
     * @param null|\Payske\Util\RandomGenerator $randomGenerator
     */
     ...
     *   <li>\Payske\Util\CaseInsensitiveArray $rheaders The response headers</li>
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (('post' === $method || 'put'=== strtolower($method)) && (Payske::$maxNetworkRetries > 0)) {
        ...
        $opts[\CURLOPT_CAINFO] = Payske::getCABundlePath();
        if (!Payske::getVerifySslCerts()) {
        ...
        // Payske's API servers are only accessible over IPv4. Force IPv4 resolving to avoid
        // potential issues (cf. https://github.com/Payske/Payske-php/issues/1045).
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case \CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Payske ({$url}).  Please check your "
                    . 'internet connection and try again.  If this problem persists, '
                    . "you should check Payske's service status at "
                    . 'https://twitter.com/Payskestatus, or';

                break;

            case \CURLE_SSL_CACERT:
            case \CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Payske's SSL certificate.  Please make sure "
            ...
            default:
                $msg = 'Unexpected error communicating with Payske.  '
                    . 'If this problem persists,';
        }
        $msg .= ' let us know at support@payske.com.';
     ...
     * @param int $errno
     * @param int $rcode
     * @param array|\Payske\Util\CaseInsensitiveArray $rheaders
     * @param int $numRetries
     *
     * @return bool
     */
    ...
    {
        if ($numRetries >= Payske::getMaxNetworkRetries()) {
            return false;
        }
        ...
        if (isset($rheaders['Payske-should-retry'])) {
            if ('false' === $rheaders['Payske-should-retry']) {
                return false;
            }
            if ('true' === $rheaders['Payske-should-retry']) {
                return true;
            }
        }
        ...
        //
        // Note that we expect the Payske-should-retry header to be false
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            Payske::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            Payske::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(Payske::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= Payske::getMaxRetryAfter()) {
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which Payske use.
