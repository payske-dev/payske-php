 ...
 * Class PayskeObject.
 */
class PayskeObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToPayskeObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_transientValues) && $this->_transientValues->includes($k)) {
            ...
            $message = "Payske Notice: Undefined property of {$class} instance: {$k}. "
                . "HINT: The {$k} attribute was set in the past, however. "
                . 'It was then wiped when refreshing the object '
                . "with the result returned by Payske's API, "
                . 'probably as a result of a save(). The attributes currently '
                . "available on this object are: {$attrs}";
            Payske::getLogger()->error($message);

            return $nullval;
        }
        $class = static::class;
        Payske::getLogger()->error("Payske Notice: Undefined property of {$class} instance: {$k}");
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof PayskeObject) {
    ...
    {
        foreach ($values as $k => $v) {
            // Special-case metadata to always be cast as a PayskeObject
            // This is necessary in case metadata is empty, as PHP arrays do
            // not differentiate between lists and hashes, and we consider
            // empty arrays to be lists.
            if (('metadata' === $k) && (\is_array($v))) {
                $this->_values[$k] = PayskeObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToPayskeObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            //   3. Its value is a PayskeObject. A PayskeObject may contain modified
            //      values within in that its parent PayskeObject doesn't know about.
            //
            $original = \array_key_exists($k, $this->_originalValues) ? $this->_originalValues[$k] : null;
            $unsaved = $this->_unsavedValues->includes($k);
            if ($force || $unsaved || $v instanceof PayskeObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if (\is_array($value)) {
            if (Util\Util::isList($value)) {
                ...
                return Util\Util::convertToPayskeObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof PayskeObject) {
    ...
    /**
     * Returns a pretty JSON representation of the Payske object.
     *
     * @return string the JSON representation of the Payske object
     */
    ...
    /**
     * Sets all keys within the PayskeObject as unsaved so that they will be
     * included with an update when `serializeParameters` is called. This
     * method is also recursive, so any PayskeObjects contained as values or
    ...
    protected static function deepCopy($obj)
    {
        ...
        if ($obj instanceof PayskeObject) {
    ...
    public static function emptyValues($obj)
    {
        if (\is_array($obj)) {
            $values = $obj;
        } elseif ($obj instanceof PayskeObject) {
            $values = $obj->_values;
        } else {
            throw new Exception\InvalidArgumentException(
    ...
    /**
     * @return null|ApiResponse The last response from the Payske API
     */
    ...
    /**
     * Sets the last response from the Payske API.
     *
