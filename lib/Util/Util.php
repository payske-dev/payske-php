...
abstract class Util
{
    ...
    /**
     * Converts a response from the Payske API to the corresponding PHP object.
     *
     * @param array $resp the response from the Payske API
     * @param array $opts
     *
     * @return array|PayskeObject
     */
    public static function convertToPayskeObject($resp, $opts)
    {
        $types = \Payske\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
                $mapped[] = self::convertToPayskeObject($i, $opts);
            }
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \Payske\PayskeObject::class;
            }
        ...
        if (null === self::$isMbstringAvailable) {
            ...
            if (!self::$isMbstringAvailable) {
                \trigger_error('It looks like the mbstring extension is not enabled. ' .
                    ...
                    'support@payske.com if you have any questions.', \E_USER_WARNING);
            }
        }
    ...
    {
        if ($h instanceof \Payske\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
