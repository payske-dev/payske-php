    ...
    public static function parse($options, $strict = false)
    {
        ...
        if (\is_string($options)) {
            if ($strict) {
                ...
                throw new \Payske\Exception\InvalidArgumentException($message);
            }
        ...
        if (\is_array($options)) {
            ...
            if (\array_key_exists('Payske_account', $options)) {
                $headers['Payske-Account'] = $options['Payske_account'];
                unset($options['Payske_account']);
            }
            if (\array_key_exists('Payske_version', $options)) {
                $headers['Payske-Version'] = $options['Payske_version'];
                unset($options['Payske_version']);
            }
        ...
        $message = 'The second argument to Payske API method calls is an '
            . 'optional per-request apiKey, which must be a string, or '
            . 'per-request options, which must be an array. (HINT: you can set '
            . 'a global apiKey by "Payske::setApiKey(<apiKey>)")';
